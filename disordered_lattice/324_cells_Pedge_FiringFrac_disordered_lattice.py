...
##########################################################################
#Arbitrary number of cells in a disordered lattice based on a triangurlar 
# lattice stimulated by a square 
#wave. Each cells has a randomly choosen hmax value
#The number of peaks past threshold are counted
#The number of edges past between nearest-neighbors are counted. 
...
ave_Pedge_list = np.array([])
std_Pedge_list = np.array([])

ave_FiringFrac_list = np.array([])
std_FiringFrac_list = np.array([])
...
while gi < np.size(g):
    ...
    while i < trials:
        ...
        #Recording time series of X molecules into a list called x_list
        jj = 0
        ...
        FiringFrac_list = np.append(FiringFrac_list, np.sum(number_of_peaks)/number_of_cells)
        ...
        Pedge_list = np.append(Pedge_list, np.sum(number_of_edges)/total_neighbor_pairs)    
    ...
    #Averaging the fraction of firing cells for all trials for a given g value
    ave_FiringFrac = np.average(FiringFrac_list)
    std_FiringFrac = np.std(FiringFrac_list, ddof=1)

    
    ave_FiringFrac_list = np.append(ave_FiringFrac_list, ave_FiringFrac)
    std_FiringFrac_list = np.append(std_FiringFrac_list, std_FiringFrac)

    
    #Averaging the edge probability for all trials for a given g value
    ave_Pedge = np.average(Pedge_list)

    std_Pedge = np.std(Pedge_list, ddof=1)
    
    ave_Pedge_list = np.append(ave_Pedge_list, ave_Pedge)
    std_Pedge_list = np.append(std_Pedge_list, std_Pedge)
...
#print(ave_Pedge_list, std_Pedge_list)
#print(ave_FiringFrac_list, std_FiringFrac_list)

np.savez("324_cells_Pedge_FiringFrac_disordered_lattice_Results", g = g, aave_Pedge_list = ave_Pedge_list, std_Pedge_list = std_Pedge_list, ave_FiringFrac_list = ave_FiringFrac_list, std_FiringFrac_list = std_FiringFrac_list)
